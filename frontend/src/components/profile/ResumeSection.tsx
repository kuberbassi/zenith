...
export const TacticalOps: React.FC = () => {
    ...
    return (
        <div className="rounded-[2.5rem] border border-white/[0.06] bg-[#0a0a0a] p-8 group hover:border-blue-500/20 transition-all duration-500 flex flex-col h-full">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-400 group-hover:scale-110 transition-transform"><Briefcase size={20} /></div>
                    <div><h3 className="text-sm font-black text-white tracking-tight uppercase">Tactical Ops</h3></div>
                </div>
                <button onClick={handleAdd} className="text-[10px] font-black text-white/20 hover:text-blue-400 uppercase tracking-widest transition-colors flex items-center gap-1"><Plus size={12} /> Add Log</button>
            </div>
                ...
                {loading ? <div className="text-white/20 text-xs">Loading...</div> : experiences.length === 0 ? <div className="text-white/20 text-xs text-center py-4">No tactical operations logged.</div> : experiences.map(exp => (
                    <div key={exp._id} className={`p-4 rounded-2xl bg-white/[0.02] border border-white/[0.04] relative group/item ${!exp.current ? 'opacity-50 hover:opacity-100 transition-opacity' : ''}`}>
                        <div className="flex items-center justify-between mb-1 pr-6">
                            ...
                            <span className={`text-[8px] font-bold uppercase tracking-widest ${exp.current ? 'text-blue-500' : 'text-white/20'}`}>{exp.current ? 'Active' : 'Completed'}</span>
                        </div>
...
export const SkillArsenal: React.FC = () => {
    ...
    return (
        <div className="rounded-[2.5rem] border border-white/[0.06] bg-[#0a0a0a] p-8 group hover:border-blue-500/20 transition-all duration-500 flex flex-col h-full">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-400 group-hover:scale-110 transition-transform"><Code size={20} /></div>
                    <div><h3 className="text-sm font-black text-white tracking-tight uppercase">Skill Arsenal</h3></div>
                </div>
                <button onClick={handleAdd} className="text-[10px] font-black text-white/20 hover:text-blue-400 uppercase tracking-widest transition-colors flex items-center gap-1"><Plus size={12} /> Adjust</button>
            </div>
            <div className="flex flex-wrap gap-2 flex-1 content-start">
                {loading ? <div className="text-white/20 text-xs">Loading...</div> : skills.length === 0 ? <div className="text-white/20 text-xs text-center w-full py-4">No skills registered.</div> : skills.map(skill => (
                    <div key={skill._id} className="relative group/tag">
                        <span className="px-3 py-1.5 rounded-xl bg-white/[0.03] border border-white/[0.06] text-[9px] font-black text-white/40 uppercase tracking-wider hover:text-blue-400 hover:border-blue-500/40 transition-all cursor-default flex items-center gap-2">
                            {skill.name}
...
export const MissionLogs: React.FC = () => {
    ...
    return (
        <div className="rounded-[2.5rem] border border-white/[0.06] bg-[#0a0a0a] p-8 group flex flex-col h-full hover:border-blue-500/20 transition-all duration-500">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-400"><Zap size={20} /></div>
                    <div><h3 className="text-sm font-black text-white tracking-tight uppercase">Mission Logs</h3></div>
                </div>
                <button onClick={handleAdd} className="text-[10px] font-black text-white/20 hover:text-blue-400 uppercase tracking-widest transition-colors flex items-center gap-1"><Plus size={12} /> Deploy</button>
            </div>
            <div className="space-y-4 flex-1">
                {loading ? <div className="text-white/20 text-xs">Loading...</div> : projects.length === 0 ? <div className="text-white/20 text-xs text-center py-4">No deployments recorded.</div> : projects.map(proj => (
                    <div key={proj._id} className="relative group/item flex items-center justify-between p-4 rounded-2xl border border-white/[0.04] bg-white/[0.01] hover:bg-blue-500/[0.02] hover:border-blue-500/20 transition-all cursor-pointer overflow-hidden">
                        ...
                        <button onClick={(e) => handleDelete(proj._id!, e)} className="absolute right-4 text-white/10 hover:text-red-400 opacity-0 group-hover/item:opacity-100 transition-all bg-[#0a0a0a] pl-2"><Trash2 size={14} /></button>
                    </div>
                ))}
            </div>
        </div>
...
export const StrategicHonors: React.FC = () => {
    ...
    return (
        <div className="rounded-[2.5rem] border border-white/[0.06] bg-[#0a0a0a] p-8 group flex flex-col h-full hover:border-blue-500/20 transition-all duration-500">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-400"><Award size={20} /></div>
                    <div><h3 className="text-sm font-black text-white tracking-tight uppercase">Strategic Honors</h3></div>
                </div>
                <button onClick={handleAdd} className="text-[10px] font-black text-white/20 hover:text-blue-400 uppercase tracking-widest transition-colors flex items-center gap-1"><Plus size={12} /> Import</button>
            </div>
                ...
                {loading ? <div className="text-white/20 text-xs">Loading...</div> : certs.length === 0 ? <div className="text-white/20 text-xs text-center py-4">No honors acquired yet.</div> : certs.map(cert => (
                    <div key={cert._id} className="group/item flex items-center justify-between gap-3 text-[10px] font-bold text-white/40 uppercase tracking-tighter">
                        <div className="flex items-center gap-3 min-w-0">
                            <ShieldCheck size={12} className="text-blue-500/40 shrink-0" />
